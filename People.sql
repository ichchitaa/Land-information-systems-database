...
DROP TABLE IF EXISTS application_lookup;
CREATE TABLE application_lookup (
    applicant_full_name_english                                               VARCHAR(100) NOT NULL,
    applicant_full_name_arabic                                                CHAR(100),
    application_id                                                            VARCHAR(50) NOT NULL,
    land_id                                                                   VARCHAR(50), 
    Previously_applied_num                                     INTEGER,
    Previously_granted_land_num                               INTEGER,
    legal_deed_id                                                  VARCHAR(50) NOT NULL COMMENT 'Let it be 0 if there is no legal deed in the application.',
    legal_deed_approved                                                       DOUBLE, 
    eb_id                                      VARCHAR(50) NOT NULL COMMENT '0 if this wasn''t submitted',
    electricity_bill_approved                                                 DOUBLE, 
    poa_id                                    VARCHAR(50) NOT NULL COMMENT '0 if no Power of Attorney.',
...
DROP TABLE IF EXISTS id_type_lookup;
...
CREATE TABLE insurance_benefits ( 
    health_insurance_ip_id VARCHAR(40) NOT NULL,
    benefit_name                                        VARCHAR(50),
    additional_info                                     VARCHAR(50)
);

DROP TABLE IF EXISTS insurance_company_lookup;
...
DROP TABLE IF EXISTS inventory_well;
...
ALTER TABLE inventory_well ADD CONSTRAINT inventory_well_pk PRIMARY
KEY ( Inventory_registration_ID );

DROP TABLE IF EXISTS involved_parties;
...
DROP TABLE IF EXISTS ip_id_numbers;
CREATE TABLE ip_id_numbers ( 
    ip_id VARCHAR(40) NOT NULL,
    id_type_lookup_id_type_id          CHAR(50) NOT NULL,
    id_number                          VARCHAR(25)
);

ALTER TABLE ip_id_numbers ADD CONSTRAINT ip_id_numbers_pk PRIMARY KEY ( ip_id );

DROP TABLE IF EXISTS involved_party_type_lookup;
...
DROP TABLE IF EXISTS land_use_lookup;
...
DROP TABLE IF EXISTS legal_deed;
...
DROP TABLE IF EXISTS setbacks_lookup;
...
CREATE TABLE university (  
    university_lookup_university_id    VARCHAR(50) NOT NULL,  
    ip_id VARCHAR(40) NOT NULL
);

ALTER TABLE university ADD CONSTRAINT university_pk PRIMARY KEY ( university_lookup_university_id,
                                                                  ip_id );

DROP TABLE IF EXISTS university_lookup;
...
ALTER TABLE application_lookup
    ADD CONSTRAINT application_lookup_electricity_bill_fk FOREIGN KEY ( eb_id )
        REFERENCES electricity_bill ( eb_id );

ALTER TABLE application_lookup
    ADD CONSTRAINT application_lookup_inv_fk FOREIGN KEY ( Inventory_registration_ID )
        REFERENCES inventory_well ( Inventory_registration_ID );

ALTER TABLE application_lookup
    ADD CONSTRAINT application_lookup_legal_deed_fk FOREIGN KEY ( legal_deed_id )
        REFERENCES legal_deed ( legal_deed_id );

ALTER TABLE application_lookup
    ADD CONSTRAINT application_lookup_power_of_attorney_fk FOREIGN KEY ( poa_id )
...
ALTER TABLE health_insurance
    ADD CONSTRAINT health_insurance_insurance_company_lookup_fk FOREIGN KEY ( insurance_company_lookup_id )
        REFERENCES insurance_company_lookup ( company_id );

ALTER TABLE health_insurance
    ADD CONSTRAINT health_insurance_involved_parties_fk FOREIGN KEY ( ip_id )
        REFERENCES involved_parties ( involved_party_id );

ALTER TABLE insurance_benefits
    ADD CONSTRAINT insurance_ben_health_fk FOREIGN KEY ( health_insurance_ip_id )
        REFERENCES health_insurance ( ip_id );

ALTER TABLE involved_parties
    ADD CONSTRAINT involved_part_app_lookup_fk FOREIGN KEY ( application_lookup_applicant_full_name_english )
...
ALTER TABLE involved_parties
    ADD CONSTRAINT involved_parties_education_level_lookup_fk FOREIGN KEY ( education_level_lookup_id )
        REFERENCES education_level_lookup ( education_level_id );

ALTER TABLE involved_parties
    ADD CONSTRAINT involved_parties_type_fk FOREIGN KEY ( involved_party_type_id )
        REFERENCES involved_party_type_lookup ( involved_party_type_id );

ALTER TABLE involved_parties
    ADD CONSTRAINT involved_parties_job_fk FOREIGN KEY ( job_title_lookup_job_title_id )
...
ALTER TABLE involved_parties
    ADD CONSTRAINT involved_p_nationality_fk FOREIGN KEY ( nationality_lookup_nationality_id )
        REFERENCES nationality_lookup ( nationality_id );

ALTER TABLE ip_id_numbers
    ADD CONSTRAINT involved_party_id_fk FOREIGN KEY ( id_type_lookup_id_type_id )
        REFERENCES id_type_lookup ( id_type_id );

ALTER TABLE ip_id_numbers
    ADD CONSTRAINT involved_party_id_fk FOREIGN KEY ( ip_id )
        REFERENCES involved_parties ( involved_party_id );
...
ALTER TABLE ownership_history
    ADD CONSTRAINT ownership_real_estate_tx_fk FOREIGN KEY 
    ( real_estate_transaction_id )
